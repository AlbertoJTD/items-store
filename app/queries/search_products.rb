class SearchProducts
  ...
  def call(params = {})
    ...
    scoped = filter_by_category_min_price(scoped, params[:min_price])
    scoped = filter_by_category_max_price(scoped, params[:max_price])
  ...
  def filter_by_category_min_price(scoped, min_price)
    return scoped unless min_price.present?

    scoped.where("price >= #{min_price}")
  end

  def filter_by_category_max_price(scoped, max_price)
  ...
  def filter_by_favorites(scoped, favorites)
    ...
    scoped.joins(:favorites).where({ favorites: { user_id: Current.user } })
  end

  def sort(scoped, order_by)
    return scoped unless order_by.present?

    order_type = Product::ORDER_TYPE.fetch(order_by&.to_sym, Product::ORDER_TYPE[:newest])
    scoped.order(order_type)
  end
end
